...
class GaussianDiffusion(nn.Module):
    ...
    @torch.no_grad()
    def p_sample_loop_2(self, shape, cond, verbose=True, return_chain=False, sample_fn=default_sample_fn, **sample_kwargs):
    ...
    @torch.no_grad()
    def p_sample_loop(self, shape, cond, verbose=True, return_chain=False, sample_fn=None, save_path='denoising_steps', **sample_kwargs):
        device = self.betas.device
        ...
        for i in reversed(range(0, self.n_timesteps)):
            t = torch.full((batch_size,), i, device=device, dtype=torch.long)
            x, values = sample_fn(self, x, cond, t, **sample_kwargs) if sample_fn else (x, None)
            x = apply_conditioning(x, cond, self.action_dim)

            # Save the denoising step as an image
            self.save_denoising_step(x, i, save_path)
            #print("XX",x.shape,flush=True)

            progress.update({'t': i, 'vmin': values.min().item() if values is not None else 0, 'vmax': values.max().item() if values is not None else 0})
            if return_chain: chain.append(x)

        progress.stamp()

        x, values = self.sort_by_values(x, values) if values is not None else (x, None)
        if return_chain: chain = torch.stack(chain, dim=1)
    ...
    def sort_by_values(self, x, values):
        inds = torch.argsort(values, descending=True)
        x = x[inds]
        values = values[inds]
        return x, values
