...
#-----------------------------------------------------------------------------#
def flatten(dataset, path_lengths=None):
    if dataset is None:
        raise ValueError("Dataset cannot be None. Please provide a dataset for normalization.")
    ...
    for key, xs in dataset.items():
        ...
        for i, x in enumerate(xs):
            if path_lengths is not None:
                ...
                flattened[key].append(x[:path_len])
            else:
                flattened[key].append(x)
        flattened[key] = np.concatenate(flattened[key], axis=0)
    return flattened
class DatasetNormalizer:
    def __init__(self, dataset, normalizer, path_lengths=None):
        # Check if dataset is a dictionary
        if isinstance(dataset, dict):
            # Flatten the dataset using path_lengths
        ...
        for key, data in dataset.items():
            ...
            self.std[key] = data.std(axis=0) + 1e-8  # To prevent division by zero
